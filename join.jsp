...
<body>
	...
	<script>
		function join() {
			...
			if(id.length > 20){
				alertNFocus("아이디는 20자 이하여야 합니다.", "[name=id]");
				return false;
			}else if(pw.length < 6 || pw.length > 20){
				alertNFocus("비밀번호는 6자 이상 20자 이하여야 합니다.", "[name=pw]");
				return false;
			}else if(name.length < 1 || name.length > 20){
				alertNFocus("이름은 1자 이상 20자 이하여야 합니다.", "[name=name]");
				return false;
			}
			...
			return false; // 홈태그가 동작을 하지 않게 해줌?
		}
